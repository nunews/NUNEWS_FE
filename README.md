...
#### 스크랩 기능 구현
- 뉴스 스크랩 시 사용자별 스크랩 데이터 저장 및 마이페이지 반영
- 스크랩 추가 및 삭제 로직 구현으로 실시간 상태 동기화
- 스크랩한 뉴스 목록을 카테고리별로 분류하여 조회 가능하도록 구성

#### 작성 글 관리 기능
- 사용자가 작성한 게시글을 마이페이지에서 별도로 조회 가능하도록 구현
- 게시글 작성 시 사용자 정보와 연동하여 자동으로 ‘내가 작성한 글’ 목록에 추가

#### 내 정보 수정 기능
- 닉네임, 프로필 이미지 등 사용자 정보 수정 기능 구현
- 수정된 사용자 정보가 전역 상태 및 마이페이지 UI에 즉시 반영되도록 처리

### 🐧 구민지

### 게시글 추가 기능
- 커뮤니티에 제목, 카테고리, 이미지, 내용을 포함한 게시글 작성 기능 구현
- supabase storage에 이미지 업로드 후 URL 저장 및 조회 흐름으로 구성

### 카테고리별 / 좋아요,최신순 정렬 기능
- 게시글을 카테고리별로 필터링 구현
- 좋아요 수와 작성일 기준으로 정렬 기능 구현

### 좋아요, 조회수, 댓글 기능
- 사용자간 상호작용을 위한 좋아요 버튼, 조회수 집계, 댓글 추가/수정/삭제 기능 구현
- 즉각적인 UI 반영을 위해 낙관적 업데이트 적용
